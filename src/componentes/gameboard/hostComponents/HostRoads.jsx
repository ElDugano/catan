...
export default function HostRoads( props ) {
  ...
  const [roadImages, setRoadImages] = useState([null,null,null,null,null,null,null,null,null,null]);
  ...
  useEffect(() => {
    const loadImages = async () => {
      ...
      for (let x in tileCornerNodes) {
        if ("rightRoadOwner" in tileCornerNodes[x][props.y] &&
          ...
          if (Number.isInteger(tileCornerNodes[parseInt(x)][props.y].rightRoadOwner)) {
            ...
            imagePath.push('./assets/roads/'+playerColor[tileCornerNodes[x][props.y].rightRoadOwner].toLowerCase()+'/'+fileName+'.png');
          }
        }
        if ("bottomRoadOwner" in tileCornerNodes[x][props.y]) {
          if (Number.isInteger(tileCornerNodes[parseInt(x)][props.y].bottomRoadOwner)) {
            ...
            imagePath.push('./assets/roads/'+playerColor[tileCornerNodes[x][props.y].bottomRoadOwner].toLowerCase()+'/'+fileName+'.png');
          }
        }
      }
      ...
      setRoadImages(importedImages);
    };

    loadImages();
  }, [tileCornerNodes, props.y, playerColor])
        ...
        if (Number.isInteger(tileCornerNodes[parseInt(x)][props.y].rightRoadOwner)) {
          boardContent.push(
            <image
              ...
              href={roadImages[tileNumber]}
              transform={"translate(" + rightRoadOffset[x][props.y].x + "," + rightRoadOffset[x][props.y].y + ")"}
              width={'224'}
              height={'80'} />
            )
          tileNumber++;
        ...
        if (Number.isInteger(tileCornerNodes[parseInt(x)][props.y].bottomRoadOwner)) {
          boardContent.push(
            <image
              key={crypto.randomUUID()}
              href={roadImages[tileNumber]}
              transform={"translate(" + downRoadOffset[x][props.y].x + "," + downRoadOffset[x][props.y].y + ")"}
              width={'208'}
              height={'96'} />
          )
          tileNumber++;
