...
export default function BoardDisplay({landTiles, landTileNumbers, thiefLocation, tileCornerNodes}) {
  ...
  for (let x=1; x <= 12; x++) {
    for (let y=0; y <= 7; y++) {
      if(tileCornerNodes[x][y].value == "Land")
      {
        if(x%2 !== 0 && y%2 == 0)
          boardContent.push(<circle key={Math.random()} id={"tileCornerNode-"+x+"-"+y} r="5" cx={defaultX + 30*x + 30} cy={defaultY + 50 * y + 0} className="cornerNodeBuildable" />);
        else if(x%2 == 0 && y%2 !== 0)
          boardContent.push(<circle key={Math.random()} id={"tileCornerNode-"+x+"-"+y} r="5" cx={defaultX + 30*x + 30} cy={defaultY + 50 * y + 0} className="cornerNodeBuildable" />);
        else
          boardContent.push(<circle key={Math.random()} id={"tileCornerNode-"+x+"-"+y} r="5" cx={defaultX + 30*x + 30} cy={defaultY + 50 * y + 20} className="cornerNodeBuildable" />);
      }
    }
  }
